...
dacura.candidate.getDecisionBasicText = function (dcm, test, type){
...
dacura.candidate.getErrorDetailsHTML = function(dcm){
...
dacura.candidate.getWarningsHTML = function(dcm){
	var html = "";
...
dacura.candidate.getUpdateDetailsHTML = function(dcm, test){
...
dacura.candidate.getJSONFragmentHTML = function(frag, title){
	html = "<div class='json-fragment-title'>"+ title + "</div><div class='dacura-json-viewer'>" + JSON.stringify(frag, 0, 4) + "</div>";
	return html;
}

dacura.candidate.getCandidateGraphUpdateHTML = function(cupdates, done){
...
dacura.candidate.getTripleTableHTML = function(trips, tit, isquads, cls){
	var html = "";
	if(trips.length > 0){
...
dacura.candidate.getGraphTestResultsHTML = function(res, dcm, type){
	...
	if(type == "Update Candidate" && typeof dcm.result != "undefined" && dcm.result != null){
		...
		if(res.decision != "reject"){
			...
			html += "<div class='title'>Candidate Cannot Currently Be Published due to Quality Control Problems</div>";
...
dacura.candidate.showFetchFailure = function (dcm, jq, type){
	var html = "<div class='api-decision-decision error'><span class='api-decision-title'>Failed to retrieve " + type +"</span></div>";	
	html +=	"<span class='api-msg-title'>" + dcm.msg_title + "</span>";
	if(typeof dcm.msg_body != "undefined" && dcm.msg_body.length > 0){
		html += "<span class='api-msg-body'>" + dcm.msg_body + "</span>";
	}
	$(jq).html("<div class='dacura-user-message-box dacura-error'>"+ html + "</div>").show();
}

dacura.candidate.showUpdateDecision = function(dcm, test, jq){
	return this.showDecision(dcm, test, jq, "Update Candidate");
}

dacura.candidate.showCreateDecision = function(dcm, test, jq){
	return this.showDecision(dcm, test, jq, "Create Candidate");
}


dacura.candidate.showDecision = function(dcm, test, jq, type){
