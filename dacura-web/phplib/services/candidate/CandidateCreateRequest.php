...
class CandidateCreateRequest extends EntityCreateRequest {
	...
	function loadFromAPI($obj){
		...
		foreach($this->ldprops as $g => $ents){
			if(!$this->schema->hasGraph($g)){
				return $this->failure_result("No such graph as $g", 400);
			}
		}
		if(!isset($this->ldprops['main'])){
			return $this->failure_result("The main graph must be present in an entity create request", 400);
		}
		$type = $this->getObjectType($this->ldprops['main']);
		if(!$type){
			return $this->failure_result("Instance data must specify the class of the entity being created (in the main graph)", 400);
		}
