...
class UserManager extends DacuraController {
	...
	function createAnonymousUser($from){
		$id = uniqid("anon");
		$u = new DacuraUser($id, $from, false, "anonymous");
		$_SESSION[$this->user_var]=& $u;
		$u->recordAction("system", "all", "anonymous login $id");
		return $u;
	}
	...
	function getAsURL(DacuraUser $u, $current_collection){
		$cols = $u->getCollectionsWithRole();
		if(count($cols) == 0 || in_array($current_collection, $cols)){
			$cid = $current_collection;
		}
		else {
			$cid = $cols[0];
		}
